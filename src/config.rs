...
impl Config {
    ...
    pub fn from_map(vars: &HashMap<String, String>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            database_url,
            database_max_connections: parse_u64("DATABASE_MAX_CONNECTIONS", 10)? as u32,
            jwt_secret,
            ...
            rate_limit_requests: parse_u64("RATE_LIMIT_REQUESTS", 120)? as u32,
            rate_limit_window: Duration::from_secs(parse_u64("RATE_LIMIT_WINDOW_SECS", 60)?),
            auth_rate_limit_requests: parse_u64("AUTH_RATE_LIMIT_REQUESTS", 10)? as u32,
            cors_allowed_origins,
            request_body_limit_bytes: parse_u64("REQUEST_BODY_LIMIT_BYTES", 1024 * 1024)? as usize,
            tls_cert_path,
            tls_key_path,
            log_json: get_or(
                "LOG_JSON",
